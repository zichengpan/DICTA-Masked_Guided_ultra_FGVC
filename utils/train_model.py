...
def train(Config,
    ...
    for epoch in range(start_epoch,epoch_num-1):
        ...
        for batch_cnt, data in enumerate(data_loader['train']):
            ...
            if Config.use_backbone and Config.two_mask_guide:
                inputs, labels, mask, sec_mask= data
                inputs = Variable(inputs.cuda())
                mask = Variable(mask.cuda())
                sec_mask = Variable(sec_mask.cuda())
                labels = Variable(torch.from_numpy(np.array(labels)).cuda())
            elif Config.use_backbone:
            ...
            if Config.two_mask_guide:
                outputs = model(inputs, mask, sec_mask, None)
            else:
                outputs = model(inputs, mask, None, None)
            ...
            if Config.mask_guide and Config.two_mask_guide:
                mse_loss_ = mse(outputs[1], outputs[2])
                mse_loss_two = mse(outputs[3], outputs[4])
                loss += 0.1 * mse_loss_ + 0.1*mse_loss_two
            elif Config.mask_guide:
                mse_loss_ = mse(outputs[1], outputs[2])
                loss += 0.2 * mse_loss_
            ...
            if Config.use_backbone:
                print('step: {:d} loss=ce_loss+swap_loss+law_loss: {:6.4f} = {:6.4f} '.format(epoch, loss.detach().item(), ce_loss.detach().item()), flush=True)
            ...
            if step % checkpoint == 0:
                ...
                if val_acc1 > highest_acc:
                    torch.save(model, 'best.pt')
                    highest_acc = val_acc1
