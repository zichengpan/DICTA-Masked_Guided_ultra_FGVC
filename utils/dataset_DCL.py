...
class dataset(data.Dataset):
    ...
    def __getitem__(self, item):
        ...
        mask_path = os.path.join("./datasets/disease_mask", os.path.basename(self.paths[item]))
        if not os.path.isfile(mask_path):
            mask = Image.open(img_path).convert('1')
        else:
            mask = Image.open(mask_path).convert('1')  # for btf dataset
        sec_mask_path = os.path.join("./datasets/disease_mask", os.path.basename(self.paths[item]))
        # sec_mask = Image.open(sec_mask_path).convert('1')  # for btf dataset
        if not os.path.isfile(mask_path):
            sec_mask = Image.open(img_path).convert('1')
        else:
            sec_mask = Image.open(sec_mask_path).convert('1')  # for btf dataset
        ...
        if self.train:
            ...
            mask = self.common_aug(mask)
            mask = self.totensor(mask)

            img_unswap = self.totensor(img_unswap)
            if self.cfg.two_mask_guide:
                sec_mask = self.common_aug(sec_mask)
                sec_mask = self.totensor(sec_mask)
                return img_unswap, label, mask, sec_mask, self.paths[item]
            ...
            return img_unswap, img_swap, label, label_swap, swap_law1, swap_law2, mask, self.paths[item]
        else:
            label = self.labels[item]
...
def collate_fn4backbone(batch):
    ...
    for sample in batch:
        ...
        if len(sample) == 8:
            label.append(sample[2])
            mask.append(sample[6])

        elif len(sample) == 5:
            label.append(sample[1])
            mask.append(sample[2])
            sec_mask.append(sample[3])
        elif len(sample) == 3:
            label.append(sample[1])

    if len(sec_mask) != 0:
        return torch.stack(imgs, 0), label, torch.stack(mask, 0), torch.stack(sec_mask, 0)
    elif len(mask) != 0:
        return torch.stack(imgs, 0), label, torch.stack(mask, 0)
    else:
